...
```
To find the best model, we retrain the network using progressively higher threshold for the partial derivatives of the error function, i.e. we stepwise incresingly train the model more. We then test the model on the validation set to see when the SSE for the predictions will start to rise, signaling that the model has become overfitted.
